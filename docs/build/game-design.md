...
3. **Spine per-beat citations.** **RULED (R-S, 2026-07-10):** keep the flat interim — the spine cites evidence as a whole via the existing `citedEvidenceIds[]`, and every inked beat renders **[unproven]** until ≥1 citation exists (warn, never block). No 02 amendment. A per-beat→evidence map (drafted as a 02 diff for approval) stays a clean, isolated later upgrade, taken only when the launch story must survive per-beat scrutiny. Interim shipped + tested 2026-07-10.
...
5. **Weather same-day retap.** **RULED (R-W, 2026-07-10): APPEND.** Every tap is recorded to `weather[]`; a second same-day tap neither replaces the entry nor locks the totem. The trough detector keeps its canon last-3-by-date window, so multiple same-day taps each count (accepted tradeoff: a bad-then-better day can shift when the Shadow is withheld). Supersedes the earlier "replace" working interim. Device-local, never transmitted.
