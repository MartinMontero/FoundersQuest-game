...
1. **Shadow divergence threshold.** Canon says "high Action + low Truth summons the Shadow" (01) with no number. Needs a ruling, e.g. Action − Truth ≥ 0.3 with ≥3 milestones raised. Until ruled, the build cannot wire the spawn condition.
2. **Council model label key.** The kickoff requires readings persist "with snapshots and their model label"; 02's `council[]` schema has no model field. Proposal: add `model` (string, e.g. `'claude-fable-5'`) to each `council[]` entry, amending 02 in the same commit. Needs approval — it is a canon schema change.
3. **Spine per-beat citations.** `s8-th` renders **[unproven]** per beat, but 02 stores one flat `citedEvidenceIds[]` per answer with no beat→evidence mapping. Needs either a ruled convention within existing keys or a 02 amendment (e.g. array-of-arrays for spine answers only).
4. **Id constants from v3 source.** `milestones[id]` ids, `sideQuests` ids, and the exact `stageId` constants must match the existing `founders-quest-v3.jsx` artifact, which is not in this repo (02 names it as the artifact source). Missing source — a CLAUDE.md stop item. Flagpoles, quest board, and `answers` keys cannot be wired until it is provided or the operator rules that this greenfield build defines the ids.
5. **Weather same-day retap.** 03: "one tap, once a day." Does a second tap on the same date replace that date's entry, or is the totem inert until tomorrow? Canon silent.
6. **Settings storage wording in 02.** Audio/volume (and any future reduced-motion override toggle) persist under their own storage key via the `makeStore` ladder, following the BYOK key precedent. 02 currently names only the data key and the player key; its storage section should name this pattern when the game code lands. Confirm the pattern is acceptable before Phase 1.
7. **Council snapshot mode.** Which `buildJournalMd` mode persists in `council[].journal` — pending operator ruling, see the Phase 0 report (recommendation: compact, the journal as sent).
