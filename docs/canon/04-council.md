...
**Earned Hunch** — sentence replacement APPLIED 2026-07-11 (Mind & Myth A1 canon commit; the prompt line above now carries the earned-hunch wording). Still queued: the appended provenance-reading block (Earned hunches = compressed pattern recognition awaiting a test — Simon; wicked-domain illusion-of-validity flag; calibration-record weighting) — its verbatim text awaits `cc-prompt-earned-hunch.md` or operator approval of a posted draft (BLOCKERS B-4); never authored by inference.
